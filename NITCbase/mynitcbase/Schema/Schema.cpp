...
int Schema::openRel(char relName[ATTR_SIZE]) {
    // Call OpenRelTable::openRel to open the relation
    int ret = OpenRelTable::openRel(relName);

    // The OpenRelTable::openRel() function returns the rel-id if successful
    // A valid rel-id will be within the range 0 <= relId < MAX_OPEN
    // Any error codes will be negative
    if (ret >= 0) {
        return SUCCESS;
    }
...
int Schema::closeRel(char relName[ATTR_SIZE]) {
    // Check if the relation is the relation catalog or attribute catalog
    if (strcmp(relName, RELCAT_RELNAME) == 0 || strcmp(relName, ATTRCAT_RELNAME) == 0) {
        return E_NOTPERMITTED; // Cannot close these reserved catalogs
    }

    // Get the rel-id of the relation using OpenRelTable::getRelId()
    int relId = OpenRelTable::getRelId(relName);

    // If the relation is not open, return an error
    if (relId < 0) {
        return E_RELNOTOPEN; // Relation is not open
    }

    // Call OpenRelTable::closeRel to close the relation
